...
export async function verifyOTP(email: string, otp: string): Promise<boolean> {
  ...
  if (!storedOtp || storedOtp.length !== otp.length) {
    return false;
  }
  const isMatch = timingSafeEqual(Buffer.from(storedOtp), Buffer.from(otp));
  if (isMatch) {
...
export async function sendOTP(email: string, otp: string, type: "login" | "signup" | "reset") {
  const subject =
    type === "signup" ? "Verify your Kreotype account" :
    type === "reset" ? "Reset your Kreotype Password" :
    "Your Kreotype Login Code";
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; rounded: 8px;">
      <h2 style="font-family: 'DM Sans', sans-serif; color: #685ACA; text-align: center;">KREOTYPE</h2>
      <p>Hello,</p>
